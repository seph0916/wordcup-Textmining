...
# 실시간 뉴스 수집과 분석 
# 뉴스1
url <- "https://search.naver.com/search.naver?where=news&sm=tab_pge&query=2018%20%EC%9B%94%EB%93%9C%EC%BB%B5%20%EC%9A%B0%EC%8A%B9%ED%9B%84%EB%B3%B4&sort=0&photo=0&field=0&pd=3&ds=2018.06.01&de=2018.07.14&cluster_rank=14&mynews=0&office_type=0&office_section_code=0&news_office_checked=&nso=so:r,p:from20180601to20180714,a:all&start=51"
web <- GET(url)
web
...
wordResult <- wordResult[-c(1:2,4,5:7)]
wordResult
...
news_C <- str_replace_all(news_C,"러시아","")
...
ruleg <- graph.edgelist(rulemat[c(36:90),], directed=F)
ruleg
